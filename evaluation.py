...
def validation(model, val_dl, loss_func):
    val_loss = 0.
    truth_res = []
    pred_res = []
    ...
    return val_loss, val_acc, val_sen, val_spe, val_mcc
...
def test(model, test_dl, loss_func):
    ...
    # calculate val statistic
    test_acc, test_sen, test_spe, test_mcc, tn, fp, fn, tp = calculate_metrics(truth_res, pred_res)
    test_loss /= len(test_dl)

    return test_loss, test_acc, test_sen, test_spe, test_mcc, truth_res, pred_res, pred_porb_res
