...
class Taeyeon_Net(nn.Module):
    ...
    def __init__(self, in_channel, feature_len):
        ...
        self.cbr1 = nn.Sequential(
                            nn.Conv1d(4, 256, kernel_size=5, stride=1, dilation=1, padding=2),
                            nn.BatchNorm1d(256),
                            nn.LeakyReLU(),
        )
        ...
        self.transition_layer1 = nn.Sequential(
                                    nn.Conv1d(256, 128, kernel_size=3, stride=2, dilation=1, padding=1),
                                    nn.BatchNorm1d(128),
                                    nn.LeakyReLU(),
        ...
        self.cbr2 = nn.Sequential(
                       nn.Conv1d(128, 64, kernel_size=3, stride=1, dilation=1, padding=1),
                       nn.BatchNorm1d(64),
                       nn.LeakyReLU(),
        )

        self.transition_layer2 = nn.Sequential(
                                    nn.Conv1d(64, 32, kernel_size=3, stride=2, dilation=1, padding=1),
                                    nn.BatchNorm1d(32),
                                    nn.LeakyReLU(),
                                    # nn.AvgPool1d(kernel_size=3, stride=2, padding=1),
                                    # nn.MaxPool1d(kernel_size=3, stride=2, padding=1)
        )
        self.classifier = nn.Linear(32*(math.ceil(math.ceil(feature_len/2)/2)), 2)
...
class Taeyeon_Net(nn.Module):
    ...
    def __init__(self, in_channel, feature_len):
        ...
        self.cbr1 = nn.Sequential(
                            nn.Conv1d(4, 42, kernel_size=3, stride=1, dilation=1, padding=1),
                            nn.BatchNorm1d(42),
                            nn.LeakyReLU(),
        )
        ...
        self.transition_layer1 = nn.Sequential(
                                    nn.Conv1d(42, 42, kernel_size=3, stride=3, dilation=1, padding=1),
                                    nn.BatchNorm1d(42),
                                    nn.LeakyReLU(),
                                    # nn.AvgPool1d(kernel_size=3, stride=2, padding=1),
                                    # nn.MaxPool1d(kernel_size=3, stride=2, padding=1)
        ...
        self.classifier = nn.Linear(42*(math.ceil(math.ceil(feature_len/3)/3)), 2)
